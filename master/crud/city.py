...
from master.models.api.city_model import CityBase,CityFunctionResp, CitiesFunctionResponse
...
class CityService(BaseBackend):
    ...
    async def get_all_cities(self)->CitiesFunctionResponse:
        try:
            query = await self.session.execute(
                select(Cities).where(Cities.is_active == True)
            )
    ...
    async def update_cities(self,request: CityBase)->CityFunctionResp:
        try:
            ...
            if query_data:
                ...
                query_data.is_active = request.is_active if hasattr(request, "is_active") else True
