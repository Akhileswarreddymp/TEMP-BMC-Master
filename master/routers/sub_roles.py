...
async def add_sub_role(
    request: SubRoleBase, session: AsyncSession = Depends(get_session)
):
    ...
    if resp_data.data_exist:
        return SubRoleResponse(message="Data already exists", data=resp_data.data)

    return SubRoleResponse(message="Success", data=resp_data.data)


@router.get("/get_all_sub_roles/{status}")
async def get_all_sub_roles(status: str, session: AsyncSession = Depends(get_session)):
    resp_obj = SubRoleService(session)

    resp_data = await resp_obj.get_all_sub_roles(status)

    if resp_data.data_exist:
        return SubRoleResponses(message="Success", data=resp_data.data)

    return HTTPException(status_code=404, detail="No sub role data exists")


@router.get("/get_sub_role/{sub_role_name}")
async def get_sub_role_data(
    sub_role_name: str, session: AsyncSession = Depends(get_session)
):
    resp_obj = SubRoleService(session)
    ...
    raise HTTPException(
        status_code=404, detail=f"{sub_role_name} sub role data does not exist in db"
    )


@router.put("/update_sub_role")
async def update_sub_role(
    request: SubRoleIdBase, session: AsyncSession = Depends(get_session)
):
    resp_obj = SubRoleService(session)
    ...
    raise HTTPException(status_code=404, detail="sub role data does not exist in db")
